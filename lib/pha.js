...
exports.buildTextWiki = function(obj, callback) {
	...
	msg.text = obj.storyText.substr(msg.username.length)
	    .trim().replace(winfo[0].title, '<' + winfo[0].uri + '|' + winfo[0].title + '>');
	msg.attachments = [ {} ];

	if (winfo[0].description) {
	    msg.attachments[0].title = "wiki v" + n + ": " + winfo[0].description;
	} else {
	    msg.attachments[0].title = "wiki v" + n;
	}
	    msg.attachments[0].title_link = winfo[0].uri
	...
	if (n == 1) {
	    ...
	    msg.attachments[0].text = disparity.unifiedNoColor(winfo[1].content, winfo[0].content)
