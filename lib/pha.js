...
function getDescription(tinfo)
{
    for (var i = 0; i < tinfo.transactions.length; i++) {
	if (tinfo.transactions[i].transactionType == "description")
	    return tinfo.transactions[i];
    }
    return null;
}
...
function getTransaction(obj, tinfo)
{
	...
	for (var j = 0; j < tinfo.transactions.length; j++) {
		...
		if (tinfo.transactions[j].transactionType == "core:create")
		    return getDescription(tinfo);
...
exports.notifyManiphest = function (obj) {
	...
	if (msg.attachments[0].text && typeof(msg.attachments[0].text) == 'string')
	    slack.slack(msg, true);
    });
};

function getWikiInfo(id, callback) {
    canduit.exec('phid.query', { "phids": [ id ] }, function (err, wiki) {
	var slug = wiki[id].uri.substr("https://phabricator.healcerion.com/w/".length);
	canduit.exec('phriction.history', { 'slug': slug }, function (err, data) {
	    callback(null, data);
	});
    });
}

exports.buildTextWiki = function(obj, callback) {
    getWikiInfo(obj.storyData.objectPHID, function(err, winfo) {
	if (!winfo) {
	    console.log("!!! Bad Wiki Info massage");
	    return;
	}
	var n = winfo.length;
	var msg = {};
	msg.username = obj.storyText.split(' ')[0];
	msg.text = obj.storyText.substr(msg.username.length)
	    .trim().replace(winfo[0].title, '<' + winfo[0].uri + '|WIKI: ' + winfo[0].title + '>');
	msg.attachments = [ {} ];
...
exports.notifyWiki = function(obj) {
    exports.buildTextWiki(obj, function(err, msg) {
	slack.slack(msg, false);
    });
};
