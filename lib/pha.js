...
Pha.prototype.convSlackGit = function(obj, callback) {
    ...
    async.waterfall([
	function(callback) {
	    self.canduit.exec('diffusion.querycommits', { "phids": [ id ] },
			 function (err, info) {
	...
	msg.channel = '#git';
	msg.username = obj.storyText.split(' ')[0];
	msg.text = 'committed <' + git.repository.uri + "|" + git.repository.fullName + '>: '
	    + '<' + git.uri + '|' + git.identifier.substr(0, 10) + '>';
	msg.attachments = [{
	    text: git.summary.replace(/T\d+/, '<https://phabricator.healcerion.com/$&|$&>')
	}];
	callback(null, msg);
    });
};
