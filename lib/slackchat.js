...
Slack.prototype.send = function(msg, done) {
    ...
    if (('attachments' in msg) && ('length' in msg.attachments) && ('text' in msg.attachments[0])) {
	dm2 = self.getDMList(msg.attachments[0].text);
	assert(Array.isArray(dm2));
    }

    if (dm.length || dm2.length) {
	dm = dm.concat(dm2);
