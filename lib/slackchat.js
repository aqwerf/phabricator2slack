...
function Slack() {
	...
	self.connected = false;
	self.emit('off');
	setTimeout(function() { self.bot.startRTM(); }, 1000);
    });
}
...
Slack.prototype.send = function(msg, done) {
    ...
    if (projects) {
	_.each(projects, function(u) {
	    let m = deepcopy(msg);
	    m.channel = m.channel + '-' + u.toLowerCase();
	    logger.info('Sent project channel: ' + m.channel);
	    say(m);
	...
	var dm2 = self.getDMList(msg.attachments[0].text);
	if (dm2)
	    dm = dm.concat(dm2);
	if (dm && dm.length > 0) {
	    _.each(_.uniq(dm), function(u) {
		count++;
		let m = deepcopy(msg);
		m.channel = u;
		logger.info('Sent DM: ' + m.channel);
		say(m);
	    });
	}
    }
